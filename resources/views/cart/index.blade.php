                    ...
                    @foreach ($cartItems as $index => $item)
                        <div class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex gap-6 items-center transition hover:shadow-md"
                            ...
                            <div class="flex-1">
                                ...
                                <p class="text-sm text-gray-500 mb-2">{{ $item->product->category->name ?? 'General' }}</p>
                    ...
                    <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 sticky top-32"
                        ...
                        <div class="space-y-4 mb-6">
                            <div class="flex justify-between text-gray-600">
                                <span>Subtotal</span>
                                <span class="font-bold">Rp {{ number_format($cartItems->sum(fn($i) => $i->product->price * $i->quantity), 0, ',', '.') }}</span>
                            </div>
                            <div class="flex justify-between text-gray-600">
                                <span>Shipping</span>
                                <span class="text-green-600 font-bold text-xs uppercase bg-green-50 px-2 py-1 rounded">Free Shipping</span>
                            </div>
                            <div class="border-t border-gray-100 pt-4 flex justify-between text-lg font-bold text-hiyoucan-900">
                                <span>Total</span>
                        ...
                        <a href="{{ route('checkout.index') }}" 
                           class="w-full block text-center bg-hiyoucan-800 text-white py-4 rounded-xl font-bold hover:bg-hiyoucan-900 transition shadow-lg hover:shadow-hiyoucan-500/20 transform hover:-translate-y-0.5">
                            Proceed to Checkout
                        </a>

                        <div class="mt-4 text-center">
                            <a href="{{ route('shop.index') }}"
                                class="text-sm text-gray-500 hover:text-hiyoucan-700 font-medium">or Continue Shopping</a>
                        </div>
                    </div>
                </div>
            </div>
